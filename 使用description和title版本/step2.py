...
def read_data(cfg):
    topics = pd.read_csv('/root/使用tree进行处理的代码/topics_tree.csv')
    ...
    #使用tree和description和title
    topics['title']=topics['title']+topics['description']
    content['title']=content['title']+content['description']
    ...
    # Drop cols
    topics.drop(['description', 'channel', 'category', 'level', 'language', 'parent', 'has_content', 'length'，'tree'], axis = 1, inplace = True)
